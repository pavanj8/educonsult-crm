...
test.describe('Student applications list', () => {
  test('student sees dashboard nav link and applications list', async ({ studentPage, page }) => {
    ...
    await expect(table.getByText('University of Toronto')).toBeVisible()
    await expect(table.getByText('MSc Computer Science')).toBeVisible()
    await expect(table.getByText('University of Melbourne')).toBeVisible()
