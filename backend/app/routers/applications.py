...
def _load_target_counselor(
    ...
    *,
    ...
    """Load the target counselor for a manual reassignment.
...
def reassign_application_counselor(
    ...
    * The target counselor must exist, belong to the application's
      ...
      by passing ``branch_id=None`` to the helper.
    ...
    * 503 -- database unavailable while loading the application, the
    ...
    if payload.counselor_id is not None:
        # Branch-scoped actors must target a counselor in the same branch;
        # consultancy owners (cross-branch scope) get branch_id=None.
        target_branch_id: int | None
        if current_user.role == Role.CONSULTANCY_OWNER:
            target_branch_id = None
        else:
            target_branch_id = application.branch_id
        _load_target_counselor(
            db,
            tenant_id=application.tenant_id,
