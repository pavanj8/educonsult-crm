...
@router.patch("/{application_id}/loan-opt-in", response_model=ApplicationResponse)
def set_application_loan_opt_in(
    application_id: int,
    payload: SetLoanOptInRequest,
    current_user: Annotated[
        AuthenticatedUser,
        Depends(require_permission(Permission.APPLICATION_READ_OWN)),
    ],
    ...
    """Set the student's loan-tracking opt-in flag on an application (E36; Journey J29; issue #199).

    Powers the student-side "Opt in to loan tracking" / "Opt out of loan
    tracking" toggle on the student application row (frontend #199,
    ``LoanOptInAction.tsx``). Requirements §5: "Loans: Tracking-only
    fields (opted-in, status, amount, lender) — no separate loan officer
    workflow for v1". This endpoint owns the ``opted-in`` flag; the
    staff-side ``status / lender / amount`` fields are tracked separately
    under E37 (Journey J30) and are out of scope here.

    Permission gating is intentionally narrow: only the application's
    owning student may toggle the flag (gated on
    ``application:read_own`` so the same role check that protects
    ``GET /applications`` also protects this toggle; no staff role
    currently has ``APPLICATION_READ_OWN``, so a stray
    consultancy-owner / branch-manager / receptionist cannot
    flip the flag on a student's behalf). Staff continue to manage
    loan ``status / lender / amount`` via the E37 staff endpoint
    (Journey J30).

    Behavior:

    * Tenant scoping is enforced via :func:`_get_tenant_application`
      (cross-tenant access surfaces as 404, not 403 -- prevents
      tenant enumeration).
    * The caller must be the application's owning student:
      ``application.student_id == current_user.id``. A student
      belonging to the same tenant but to a different application
      is rejected with 403 (mirrors the E18 / E21 student-scope
      pattern; ``_get_active_student`` already verifies that the
      caller has ``tenant_id`` and is an active student account).
    * ``loan_opt_in`` is a boolean, accepted as ``true`` / ``false``
      in the request body. Both directions are persisted; the toggle
      is symmetric (a student may opt in and opt back out).
    * No stage-history row is written and no in-app notification is
      generated by this endpoint -- those surfaces are not part of
      the Journey J29 acceptance criteria and are deliberately out
      of scope for this ticket.
    * No audit-log row is written either; the ``loan_opt_in`` column
      itself records the new state and the application's
      ``updated_at`` timestamp records when the toggle happened.
      A future audit-log follow-up can back-fill from those columns.
    ...
    * 401 -- caller is not authenticated.
    * 403 -- caller is not a STUDENT (no ``APPLICATION_READ_OWN``)
      or is a STUDENT that does not own this application.
    * 404 -- application does not exist or belongs to a different
      tenant.
    * 422 -- body fails Pydantic validation (e.g. ``loan_opt_in``
      is not a boolean).
    * 503 -- database unavailable while loading the application or
    ...
    if application.student_id != student.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Students may only update their own applications",
        )

    application.loan_opt_in = payload.loan_opt_in
