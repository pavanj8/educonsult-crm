"""Tests for ``POST /applications/{id}/mark-rejected`` (E39; Journey J32; #205).

Covers the dedicated mark-rejected transition:

* happy path — an in-flight application is flipped to REJECTED and the REQUIRED
  reason is captured (trimmed) on the StageHistory row;
* reason is mandatory — missing / empty / whitespace-only is a 422;
* an application already in a terminal stage cannot be rejected (422);
...
def test_mark_rejected_flips_stage_and_captures_reason(client, db_session, override_authenticated_user):
    tenant, branch, counselor = _setup(db_session, "rej-ok")
    ...
    assert body["history_entry"]["to_stage"] == PipelineStage.REJECTED.value
    assert body["history_entry"]["reason"] == "Documents did not meet requirements"  # trimmed

    db_session.expire_all()
    assert db_session.get(Application, application.id).stage == PipelineStage.REJECTED
...
def test_mark_rejected_requires_a_reason(client, db_session, override_authenticated_user):
    tenant, branch, counselor = _setup(db_session, "rej-req")
    application = _seed_app(db_session, tenant=tenant, branch=branch, counselor=counselor)
    override_authenticated_user(_as(Role.COUNSELOR, counselor))
    url = f"/applications/{application.id}/mark-rejected"
    headers = {"Authorization": "Bearer test-token"}
...
def test_mark_rejected_terminal_stage_is_422(client, db_session, override_authenticated_user):
    tenant, branch, counselor = _setup(db_session, "rej-terminal")
...
def test_mark_rejected_denied_for_non_advance_roles(client, db_session, override_authenticated_user, role):
    tenant, branch, counselor = _setup(db_session, f"rej-authz-{role.value}")
    application = _seed_app(db_session, tenant=tenant, branch=branch, counselor=counselor)
...
def test_mark_rejected_cross_tenant_is_404(client, db_session, override_authenticated_user):
    tenant, branch, counselor = _setup(db_session, "rej-home")
    application = _seed_app(db_session, tenant=tenant, branch=branch, counselor=counselor)
    other = _create_tenant(db_session, name="rej-other", slug="rej-other")
...
def test_mark_rejected_counselor_other_branch_is_403(client, db_session, override_authenticated_user):
    seed_default_stage_transitions(db_session)
    tenant = _create_tenant(db_session, name="rej-branch", slug="rej-branch")
